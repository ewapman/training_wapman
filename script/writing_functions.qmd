...
air_temps_c <-convert_f_to_c(airtemps)
```
### Exercise: convert celsius to fahrenheit
```{r}
convert_c_to_f <- function(cel) {
  fahr <-  (cel * 9 / 5) + 32
  return(fahr)
}
convert_c_to_f(air_temps_c)
```
```{r}
...
```{r}
#default temp set at 70
calc_hotcold <- function(df, thresh = 70) {
  if(!'fahr' %in% names(df)) stop('The data frame must contain a column called fahr')
  
  out_df <- df %>% 
    mutate(hotcold = if_else(condition = (fahr > threshold),
                            true = 'hot', 
                            false = 'cold'))
  return(out_df)
}

#tidyverse automatically passes last df into next function after pipe
f_temps_df %>% 
  select(fahr, celsius) %>% 
  calc_hotcold(thresh = 55) %>% #automatically passes last dataframe 
  arrange(desc(fahr))
```

### Create a custom theme for ggplot()
```{r}
# Added :: for ggplot incase someone didn't load ggplot library
...
ggplot(f_temps_df, 
       aes(x = fahr, y = celsius, color = kelvin)) +
  geom_point() +
